"""Engine API (MISSION.md §12, G1) — the callable core `app/` and `rebuild.py` both consume.

`analyze()` loads/repairs a mesh and reports frame/extent/body-count WITHOUT building geometry.
`rebuild()` runs the full pipeline, optionally reporting progress and honoring cancellation.
Both are thin wrappers around `_rebuild_argparse` (the argparse-driven pipeline moved here
verbatim from `pipeline/cli.py::_run`) so CLI behaviour is byte-identical to before the move —
`pipeline/cli.py::_run` now just calls `_rebuild_argparse` directly.
"""
import argparse
import contextlib
import io
...
class RebuildError(Exception):
    """Base for typed exceptions `rebuild()` raises, mirroring `rebuild.py` CLI exit codes."""
    def __init__(self, message: str, exit_code: int):
        super().__init__(message)
        self.exit_code = exit_code


class UsageOrCrashError(RebuildError):
    """Mirrors CLI exit code 2 — bad args or an unexpected internal crash."""
    def __init__(self, message: str):
        super().__init__(message, exit_code=2)


class InputError(RebuildError):
    """Mirrors CLI exit code 3 — the input mesh itself is unusable (not watertight, etc)."""
    def __init__(self, message: str):
        super().__init__(message, exit_code=3)


class TopologyError(RebuildError):
    """Mirrors CLI exit code 4 — an unsupported cross-section topology was encountered."""
    def __init__(self, message: str):
        super().__init__(message, exit_code=4)


class GeometryError(RebuildError):
    """Mirrors CLI exit code 5 — the final solid failed validity checking."""
    def __init__(self, message: str):
        super().__init__(message, exit_code=5)


class RebuildCancelled(Exception):
    """Raised from inside `rebuild()` when the caller's `cancel()` callback returns True."""


_EXIT_CODE_TO_EXC = {2: UsageOrCrashError, 3: InputError, 4: TopologyError, 5: GeometryError}


@dataclass
class Analysis:
    """Result of `analyze()` — everything the GUI needs to populate the Outline/Details
    docks and pre-fill the Run controls before any geometry is built."""
    frame_axis: list          # unit vector, motor axis in the input STL's own frame
    origin_xy_mm: list        # [ox, oy] the on-axis origin found in the input STL's own frame
...
def _auto_axis_confidence(mesh) -> float:
    """Cheap, self-contained re-derivation of io._auto_axis's eigengap, expressed as a 0..1
    confidence: 1.0 == the distinct axis is unambiguous, near 0 == any of the three directions
    would have fit about as well (a near-spherical or cube-like body)."""
    ...
    spread = float(eigvals[-1] - eigvals[0])
    if spread <= 0:
        return 0.0
...
class RebuildOptions:
    """Mirrors `rebuild.py`'s argparse namespace (`pipeline/cli.py::_parse_args`) — the same
    fields, same defaults, so `rebuild()` can hand this straight to `_rebuild_argparse`."""
    input_stl: str
    output: str
    axis: str = "z"
    units: str = "mm"
    sections: int = 40
    refine_bands: Optional[str] = None
    adaptive: bool = False
    chord_tol: float = 0.5
...
def rebuild(opts: RebuildOptions, on_progress: Optional[Callable[[str, float, str], None]] = None,
    ...
    """Run the full pipeline. `on_progress(stage, frac, message)` is called at coarse stage
    boundaries and once per station. `cancel()` is polled between stations; if it returns True,
    `RebuildCancelled` is raised. On failure, raises a `RebuildError` subclass whose
    `.exit_code` mirrors the `rebuild.py` CLI exit code (2-5); on success, returns a `Result`.

    This is a wrapper, not a reimplementation: it builds the same argparse-shaped namespace
    `pipeline/cli.py` has always used and calls `_rebuild_argparse` — the exact code moved here
    from `cli.py::_run` — so behaviour is byte-identical to the CLI. Only the two hook points
    added directly in `_rebuild_argparse` (station-loop start, and the end of the function) are
    new: they are no-ops (`getattr(args, ..., None) is None`) on the plain CLI path."""
    ...
    try:
        ...
        raise UsageOrCrashError(f"{type(exc).__name__}: {exc}\n" + "\n".join(tb)) from exc
    if code != 0:
        exc_cls = _EXIT_CODE_TO_EXC.get(code, UsageOrCrashError)
        detail = stderr_buf.getvalue().strip() or f"exit code {code} (no stderr captured)"
        raise exc_cls(detail)
    rep = _load_report(args.report) if args.report else None
...
def _rebuild_impl(args) -> int:
    ...
    _cancel = getattr(args, "cancel", None)
    _n_zs = len(zs)
    for _i, z in enumerate(zs):
        if _cancel is not None and _cancel():
            raise RebuildCancelled(f"cancelled at station {_i}/{_n_zs} (z={z:.3f})")
    ...
    if _on_progress_end is not None:
        _on_progress_end("done", 1.0, "rebuild complete")
    return 0
