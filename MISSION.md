...
| **M16** | **non-proportional tapered-bore station geometry** (dome-pinch-override fix + non-pinch curved-endpoint snap + actual-rings loft, `docs/plans/tapered_bore_dome_pinch_and_surface_area.md`) | Capsule L=10000/R_o=1000 (2:1 domes), flat-capped at z=150/9850 (envelope 714.1 mm, well inside real dome curvature — NOT an island pinch); circular bore R=300 fore of z=6000 (single M4-style event, flat fore wall); two-family alternating 10-lobe star (`n_pairs=5`) aft of it, lofted z=6000→10010 (past the aft cap), tip families 480→620 and 400→460 (ratio drifts 1.200→1.348 — no scalar maps one end onto the other, defeating the proportional loft), valley 380→410 (widened from an initial 330→360 draft — see `harness/milestones.py::_m16` for the measured `gmsh_tet` reason). Clean tessellation (ct=0.5) — tests geometry, not real-scan noise robustness (explicitly deferred, plan §4.1). | volume < 0.3 %; dev max < 2·ct, p99 < 0.8·ct; dome_stations_min 8; bbox_err_pct 0.1 (the dome-pinch-override tripwire); `topo_events_z_mm = [6000]`, max 3; face_count ≤ 200; gmsh ≥ 0.1; runtime < 600 s | `--axis z --sections 160 --chord-tol 0.5` |
...
"normalize" these back to a chord_tol ratio, that would make them unpassable; M16 only the
non-proportional tapered-bore station geometry (both flat-capped ends and the single circular-
to-star event already exist in M4/M14's own shapes — M16's new stress is exclusively the
aft zone's genuinely non-proportional growth, which the pre-existing bore-loft path had never
been asked to represent and never validated against). Why cosine end-clustering cannot pass M8: at n=80 the
